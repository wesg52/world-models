...
do
    ...
    do
        sbatch slurm_openmind/train.sh $model world_place $EXPERIMENT_NAME $prompt coords
    done

    for prompt in "${US_PLACE_PROMPTS[@]}"
    do
        sbatch slurm_openmind/train.sh $model us_place $EXPERIMENT_NAME $prompt coords
    done

    for prompt in "${NYC_PLACE_PROMPTS[@]}"
    do
        sbatch slurm_openmind/train.sh $model nyc_place $EXPERIMENT_NAME $prompt coords
    done

    for prompt in "${ART_PROMPTS[@]}"
    do
        sbatch slurm_openmind/train.sh $model art $EXPERIMENT_NAME $prompt release_date
    done

    for prompt in "${HEADLINE_PROMPTS[@]}"
    do
        sbatch slurm_openmind/train.sh $model headline $EXPERIMENT_NAME $prompt pub_date
    done

    for prompt in "${HISTORICAL_PROMPTS[@]}"
    do
        sbatch slurm_openmind/train.sh $model historical_figure $EXPERIMENT_NAME $prompt death_year
    done
done
